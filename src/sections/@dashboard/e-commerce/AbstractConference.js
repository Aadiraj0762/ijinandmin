...
//     console.log('Form Data:', data); // Debug collected form data
//     try {
//       if (isEdit) {
...
export default function ConferenceForm({ isEdit, currentConference }) {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      reset();
      enqueueSnackbar(!isEdit ? 'Create success!' : 'Update success!');
      navigate(PATH_DASHBOARD.conference.list); // Navigate to the conference list after submission
    } catch (error) {
      console.error('Submission error:', error);
      enqueueSnackbar('An error occurred. Please try again.', { variant: 'error' });
    }
